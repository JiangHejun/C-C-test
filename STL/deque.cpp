 ...
 * @Description:
 * @Author: Hejun Jiang
 * @Date: 2021-09-22 11:54:57
 * @LastEditTime: 2021-09-22 11:54:58
 * @LastEditors: Hejun Jiang
 * @Version: v0.0.1
 * @Contact: jianghejun@hccl.ioa.ac.cn
 * @Corporation: hccl
 */
// 一、deque的中控器 deque是连续空间（至少逻辑上看来如此），连续线性空间总令我们联想到array或vector。
// array无法成长，vector虽可成长，却只能向尾端成长，而且其所谓的成长原是个假象，事实
//     上是（1）另觅更大空间；（2）将原数据复制过去；（3）释放原空间三部曲。如果不是vector
//         每次配置新空间时都有留下一些余裕，其成长假象所带来的代价将是相当高昂。
//         deque系由一段一段的定量连续空间构成。一旦有必要在deque的前端或尾端增加新空间，便
//             配置一段定量连续空间，串接在整个deque的头端或尾端。deque的最大任务，便是在这些分
//                 段的定量连续空间上，维护其整体连续的假象，并提供随机存取的借口。避开了“重新配置、
//                 复制、释放”的轮回，代价则是复杂的迭代器架构。
//                 受到分段连续线性空间的字面影响，我们可能以为deque的实现复杂度和vector相比虽不中
//                     亦不远矣，其实不然。主要因为，既是分段连续线性空间，就必须有中央控制，而为了维持
//                         整体连续的假象，数据结构的设计及迭代器前进后退等操作都颇为繁琐。deque的实现代码
//                             分量远比vector或list都多得多。
//                             deque采用一块所谓的map（注意，不是STL的map容器）作为主控。这里所谓map是一小块
//                                 连续空间，其中每个元素（此处称为一个节点，node）都是指针，指向另一段（较大的）
//                                     连续线性空间，称为缓冲区。缓冲区才是deque的储存空间主体。SGI STL 允许我们指定
//                                         缓冲区大小，默认值0表示将使用512 bytes 缓冲区。 二、deque的迭代器
//                                             让我们思考一下，deque的迭代器应该具备什么结构，首先，它必须能够指出分段连续空
//                                                 间（亦即缓冲区）在哪里，其次它必须能够判断自己是否已经处于其所在缓冲区的边缘，
//                                                     如果是，一旦前进或后退就必须跳跃至下一个或上一个缓冲区。为了能够正确跳跃，dequ
//                                                         e必须随时掌握管控中心（map）。所以在迭代器中需要定义：当前元素的指针，当前元素
//                                                             所在缓冲区的起始指针，当前元素所在缓冲区的尾指针，指向map中指向所在缓区地址的指针。
//                                                             在进行迭代器的移动时，需要考虑跨缓冲区的情况。 重载前加(
//                                                                 减) ，在实现后加(减) 时，调用重载的前加(减)。 重载
//     +=
//     , 实现 + 时，直接调用 +=,
//     实现 -= 时，调用 += 负数，实现 - 时，调用
//     -=.
//       //当需要实现新的功能时，最好使用已经重载好的操作，即方便有安全。。。。
//       另外，deque在效率上来说是不够vector好的，因此有时候在对deque进行sort的时候，需要先
//       将元素移到vector再进行sort，然后移回来。

//       构造函数：根据缓冲区设置大小和元素个数，决定map的大小；给map分配空间，根据缓冲区的个
//       数，分配缓冲区，默认指定一个缓冲区； 设置start和finish迭代器，满足左闭右开的原则。 push_back
//     : 如果空间满足，直接插入；不满足，调用push_back_aux。 push_back_aux : 先调用reverse_map_at_back,
//     若符合某种条件，重换一个map；分配空间。 reserve_map_at_back
//     : 看看map有没有满，满的话，调用reallocate_map。 reallocate_map
//     : 如果前端或后端pop过多，就会导致大量的空闲空间，如果是这种情况，则不用 新分配空间，调整一下start的位置即可；
//       如果不够，则需要重新申请空间。 pop：析构元素，如果是最后一块还需要删除空间。
//       erase：需要判断，前面的元素少还是后面的元素少，移动较少的部分。
//       insert：判断位置，如果为前端或后端直接调用push操作，否则，移动较少的一端。 deque的构造与内存管理：
//       由于deque的设计思想就是由一块块的缓存区连接起来的，因此它的内存管理会比较复杂。插入的
//       时候要考虑是否要跳转缓存区、是否要新建map节点（和vector一样，其实是重新分配一块空间给
//       map，删除原来空间）、插入后元素是前面元素向前移动还是后面元素向后面移动（谁小移动谁）
// 。而在删除元素的时候，考虑是将前面元素后移覆盖需要移除元素的地方还是后面元素前移覆盖
//       （谁小移动谁）。移动完以后要析构冗余的元素，释放冗余的缓存区。
...
class deque {
   ...
   public:  // Constructor, destructor.
    ...
    deque& operator=(const deque& x) {
        ...
        if (&x != this) {
            ...
            if (len >= x.size()) erase(copy(x.begin(), x.end(), start), finish);
   ...
   public:  // Insert
    ...
    void insert(iterator pos, int n, const value_type& x) { insert(pos, (size_type)n, x); }
    void insert(iterator pos, long n, const value_type& x) { insert(pos, (size_type)n, x); }
